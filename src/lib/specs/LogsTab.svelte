<script lang="ts">
  ...
  let hoveredEntry = $state<string | null>(null);
  ...
  {#if vscodeState.logs?.stats}
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        ...
        <span class="stat-num success">OK</span>
    ...
    {:else}
      <div class="entries-list">
        {#each filteredEntries as entry, i (entry.id)}
          ...
          <div
            class="entry-card {isExpanded ? 'entry-expanded' : ''} {isHovered ? 'entry-hovered' : ''}"
            style="animation-delay: {i * 30}ms"
            onmouseenter={() => hoveredEntry = entry.id}
                ...
                {#if entry.filesModified.length > 0}
                  <div class="files-section">
                    <div class="files-label">
                      ...
                      Modified ({entry.filesModified.length})
                    </div>
                ...
                {#if entry.filesCreated.length > 0}
                  <div class="files-section">
                    <div class="files-label">
                      ...
                      Created ({entry.filesCreated.length})
                    </div>
                    <div class="files-list">
...
<style>
  ...
  .search-input:focus { border-color: #4d9eff; box-shadow: 0 0 0 2px rgba(77, 158, 255, 0.15); }
  ...
  .spec-select:focus { border-color: #4d9eff; }
  .spec-select option { background: #252525; color: #e0e0e0; }
  ...
  .icon-logs { background: rgba(77, 158, 255, 0.1); color: #4d9eff; }
  .icon-files { background: rgba(121, 192, 255, 0.1); color: #79c0ff; }
  .icon-ok { background: rgba(63, 185, 80, 0.1); color: #3fb950; }
  ...
  .entry-expanded { border-color: #383838; }
  ...
  .entry-header:hover { background: #2a2a2a; }
  ...
  .file-count { font-size: 10px; color: #555; flex-shrink: 0; }
  ...
  .files-section { display: flex; flex-direction: column; gap: 5px; }
  ...
  .file-chip:hover { filter: brightness(1.2); }
